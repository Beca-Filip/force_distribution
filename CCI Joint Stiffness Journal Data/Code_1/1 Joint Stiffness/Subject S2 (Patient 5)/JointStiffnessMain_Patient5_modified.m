...
LoadFileName = 'results_patient5_v22f_optModel5_23456_610sigma_new.mat'; % all results for the simulation get saved here
load(LoadFileName);
...
Speeds = [0.25 0.35 0.45 0.55 0.65]; % which speeds will be loaded
Leg = 'both'; %  Determine number of gait speeds to be used in later optimizations
nMusc = 34; % Specify number of muscles and EMG signals for each muscle
SaveStiffness = 0; % 1 % save stiffness results
SaveFileName1 = 'Patient5_joint_stiffness_v22f_new.mat';
SaveFileName2 = 'Patient5_joint_stiffness_v22f_individual_mus_new.mat';
...
MuscRef(1:17) = 1; % Hip 19
MuscRef([18:21 23]) = 2; % Hip-Knee 22
MuscRef([22 24:26]) = 3; %Knee 4
MuscRef([27:28]) = 4; % Knee-Ankle 13
MuscRef([29:34]) = 5; % Ankle 10
...
%% Construc MusParams for joint stiffness calculation
...
MusParams.VmaxFactor = 10; % VmaxFactor %%%%%%

%% get the symbolic experssion of dfdl
% modelnumber = 1; % 1: model from matlab; 0: model from autolev
% dfdl_sym = dfdlSymbolicExpression(modelnumber);
