...
for leg = leg_list
    for speed = speed_list
        for trial = trial_list
            ...
            JointStiffnessMain_Patient4_modified
            ...
            load('results_patient4_v22f_optModel5_45678_610sigma_new.mat', vars_to_load{:})
...
%             J17 = - real(sum(sqrt(1-(fnormalisedfmax).^2),1)) / n; % Soft saturation
            ...
            % Dimensionality same as moment
            data.A_orig(:, :, :, trial, speed-3, leg) = A;
            for k = 1 : 101
                data.A(:, :, k, trial, speed-3, leg) = A(:,:,k)*diag(cos(alpha));
            end
            ...
            % Time-constant data
            % Dimensionality same as force
            data.pcsa(:, :, trial, speed-3, leg) = pcsa;
...
%             end
            ...
            % Store
            data.foot_off_index_m(:, :, trial, speed-3, leg) = FootoffM;
            data.foot_off_index_a(:, :, trial, speed-3, leg) = Footoffa;
...
save_dir = "Optimization Model Data\Patient4.mat";
if save_flag
    save(save_dir, "data");    
end