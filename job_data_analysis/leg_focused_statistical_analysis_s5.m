...
cmap = linspecer(length(leg_list));
cmap = linspecer(4);
cmap = cmap([3, 4], :);
% linestyles
linstyl = {'--', ':', '-.', '-'};
...
%     opts.title = @(n) {sprintf('Muscle %d: $\\sigma = %.2f$N', n, mean(std_f(n, :, :))), 'interpreter', 'latex', 'FontSize', 15};
    ...
    for leg = leg_list
        ...
        cmap_index = find(leg == leg_list);
        linstyl_index = mod(find(leg == leg_list), length(linstyl));
        
        % Plot standard deviations only on request
        if std_flag
        % Darken the color for patch
        patch_vector_quantities_opts(stdpatch_t, squeeze(stdpatch_f), [], [], [.5, .5, .5], 'EdgeColor', cmap(cmap_index, :), 'FaceAlpha', .2, 'FaceColor', cmap(cmap_index, :), 'DisplayName', sprintf('$m_{leg_{%d}} \\pm 3\\sigma_{leg_{%d}}$', leg, leg));
        end
        ...
        if (leg ~= leg_list(end))
            [h_sp, ~] = plot_vector_quantities_opts(sample_list-1, squeeze(mean_f), [], [], 'LineWidth', 2,...
                'LineStyle', linstyl{linstyl_index}, 'Color', cmap(cmap_index, :), 'DisplayName', sprintf('$m_{leg_{%d}}$', leg));
        ...
        exportgraphics(gcf, sprintf("graphs/subject5_speed%d_speed%dvs%d.pdf", speed, leg_list),'contenttype','vector');
    end
end