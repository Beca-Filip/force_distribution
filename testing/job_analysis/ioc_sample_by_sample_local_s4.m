...
% Do every leg, speed, trial and sample
for leg_select = 1:2
for speed_select = 1:5
for trial_select = 1:10
for sample_select = 1:101
    ...
    % Compute solution
    fprintf("Computing IOC %d-%d-%d.\n", sample_select, speed_select, leg_select);
    % Fout_curr = DO_subroutine_normalized(alpha0, data, vars, model, sample_list, trial_list, speed_list, leg_list);
    [alpha, err, exflg, out, lam, grad, hess] = IO_local_fmincon_search_normalized(alpha0, data, vars, model, sample_select, trial_select, speed_select, leg_select);
    
    % Save solution
    save(sprintf("ios4/%d-%d-%d.mat", sample_select, speed_select, leg_select), "alpha", "err", "exflg", "out", "lam", "grad", "hess", "sample_select", "sample_list", "trial_list", "speed_select", "leg_select");
end
end
end
end