...
data.f = reshape(f, [size(f), 1, 1, 1]);
data.fmin = reshape(fmin, [size(fmin), 1, 1, 1]);
data.fmax = reshape(fmax, [size(fmax), 1, 1, 1]);
data.pcsa = reshape(pcsa, [size(pcsa), 1, 1, 1]);
data.vmt = reshape(vmt, [size(vmt), 1, 1, 1]);
data.M = reshape(M, [size(M), 1, 1, 1]);
data.fpassive = reshape(fpassive, [size(fpassive), 1, 1, 1]);
data.f0 = reshape(f0, [size(f0), 1, 1, 1]);
data.mass = reshape(mass, [size(mass), 1, 1, 1]);
data.r = reshape(r, [size(r), 1, 1, 1]);
data.A = reshape(A, [size(A), 1, 1, 1]);
data.b = reshape(b2, [size(b2), 1, 1, 1]);
% For each sample
for k = 1 : K
...
%     model.set_initial(vars.variables.f, f(:, :, k));
    model = set_model_parameters(data, vars, model, k, 1, 1, 1);
